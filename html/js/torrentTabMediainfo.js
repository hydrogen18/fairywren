	...
	attach : function(data,div)
	{
		...
		for(filename in data.files)
		{
			...
			for(var i = 0; i != tracks.length; ++i)
			{
				...
				var showTrack = track.type in Fairywren.torrentTab.extension.mediainfo.allowedTracks;
				...
				for( k in track )
				{
					...
					if(showTrack)
					{
						showKey = k in Fairywren.torrentTab.extension.mediainfo.allowedTracks[track.type];
					}
					
					if(showKey)
					{
						kText = Fairywren.torrentTab.extension.mediainfo.allowedTracks[track.type][k]||kText;
					}
		...
		showAllButton.addClass('torrentLink');
		...
		div.append($("<div />").append(showAllButton));
		div.append(files);
	}
};

Fairywren.torrentTab.extension.mediainfo.allowedTracks = {
	'General' : {
		'Format' : null,
		'Duration' : null,
		'File_size' : 'File Size',
